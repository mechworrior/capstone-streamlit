...
def get_weather(
    ...
    def parse_headers(cols):
        new_cols = []
        for col in cols:
            new_cols.append("".join(list(dict.fromkeys(col, None).keys())[-1::-1]))
        return new_cols
    ...
    for year in params["years"]:
        for month in params["months"]:
            ...
            df_cur["日"] = pd.to_datetime(
                df_cur["日"].apply(lambda x: f"{year}-{month:02d}-{x:02d}")
            )
    ...
    for col in df_total.columns:
        df_total[col] = (
            df_total[col].astype(str).str.replace(r"[^\d.-]", "", regex=True)
        )
